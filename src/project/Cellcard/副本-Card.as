package project.Cellcard 
{
	...
	public class Card extends BaseSprite 
	{
		...
		private function init():void 
		{
			...
			/*TexturePool.getInstance().loadAssets(ModelLocator.getInstance().dynamicURL+"B/"+ "cardBg" + ".png", function(param:Texture,url:String):void {
			_bg = new Image(param);
			if(_currentCardTye!=2)
		...
		public function setCardModel(value:CardModel, updatFlag:Boolean = true, fun:Function = null):void {
			...
			_cardModle = value;
			if (_cardImage == null) {
				_cardImage = new MyImage(ApplictionConfig.CARD_WIDTH-2, ApplictionConfig.CARD_HEIGHT);
		...
		public function showMiddleImage(updatFlag:Boolean=false):void 
		{
			if (_bg) {
				_bgcontent.addChild(_bg);
				//				trace(getChildIndex(_bgcontent) )
				//swapChildren(_bg,_cardImage);
			}
			...
			_cardImage.setTexture(temp);
			temp = null;
			_cardImage.scaleX = _cardImage.scaleY = 0.64;
			/*_textSp.unflatten();
		...
		{
			_barSp.unflatten();
			_cardImage.scaleX = _cardImage.scaleY = 1;
			if(source) source.dispose();
			source= ApplictionConfig.assets.getTexture(raceBar);
