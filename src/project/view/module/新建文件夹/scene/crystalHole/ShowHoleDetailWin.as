package project.view.module.scene.crystalHole
{
	...
	public class ShowHoleDetailWin extends BaseTitleWindow
	{
		...
		{
			_mineModel = value;
			_titleMc.gotoAndStop(value.type);
			...
			if (value.owner)
			{
				...
				_occupyNameText.htmlText ="<u><a href='event:" + value.owner.userId + "'>" +value.owner.nick + "</a></u>";
			}
			else
			{
				_occupyNameText.htmlText = "";
			}
			...
			if (!isMyMine(value.owner) && value.owner )
			{
				//不是自己的矿坑,有矿主
		...
		{
			if (!isMyMine(_mineModel.owner)&& _mineModel.owner )
			{
				//不是自己的矿坑,有矿主
		...
		{
			//如果是宝石矿则不能占
			if (_mineModel.type == MineType.GEM) {
		...
		{
			_titleMc = getChild("titleMc") as MovieClip;
			_titleMc.stop();
			...
			_occupyNameText.addEventListener(TextEvent.LINK, linkHandler, false, 0, true);
		...
		/**
		 * 如果占领跟掠夺时活力不够,先判断背包是否有道具,有则提示是否使用,如果没用则提示购买物品
		 * 道具提示购买以增加2点的道具(编号95)为准
		 * @param	e
		 */
		private function showBuyTip():void {
			UseItemControler.judgestItemExistAndCallBack(SpecialItemIdConfig.DYNAMIC2, function():void {
				//实用增加2活力道具
				var userItemAddDynamic:Function = function(okOrCancel:String):void {
				if (okOrCancel==AlertSelectPane.ALERT_YES) {
					UseItemControler.userItemNew(SpecialItemIdConfig.DYNAMIC2);
					}
				}
				AlertSelectPane.getInstance().showMessage(String(LocalConfig.LOCAL_LAN.CrystalHolePane.UserItemTip.@Text).replace("{ItemName}",ItemConfig.ITEM_CONFIG.Item.(@ID == SpecialItemIdConfig.DYNAMIC2).@Name), userItemAddDynamic, true);
				//UseItemControler.userItemNew(SpecialItemIdConfig.DYNAMIC2);
				},function():void {
					//没有道具时,找增加5点的活力道具
					UseItemControler.judgestItemExistAndCallBack(SpecialItemIdConfig.DYNAMIC5, function():void {
					//实用增加5活力道具
					var userItemAddDynamic1:Function = function(okOrCancel:String):void {
					if (okOrCancel==AlertSelectPane.ALERT_YES) {
						UseItemControler.userItemNew(SpecialItemIdConfig.DYNAMIC5);
						}
					}
					AlertSelectPane.getInstance().showMessage(String(LocalConfig.LOCAL_LAN.CrystalHolePane.UserItemTip.@Text).replace("{ItemName}",ItemConfig.ITEM_CONFIG.Item.(@ID == SpecialItemIdConfig.DYNAMIC5).@Name), userItemAddDynamic1, true);
					
					},function():void {
						//没有道具时提示购买增加2点的活力道具
						var buyItem:Function = function(okOrCancel:String):void {
						if (okOrCancel==AlertSelectPane.ALERT_YES) {
							UseItemControler.showBuyWin(SpecialItemIdConfig.DYNAMIC2);
							}
						}
						AlertSelectPane.getInstance().showMessage(String(LocalConfig.LOCAL_LAN.CrystalHolePane.BuyItemTip.@Text).replace("{ItemName}",ItemConfig.ITEM_CONFIG.Item.(@ID == SpecialItemIdConfig.DYNAMIC2).@Name), buyItem, true);
		...
		private function clickHandler(e:MouseEvent):void
		{
			switch (e.currentTarget)
			{
				case _occpyBt:
					...
					if (CrystalHoleModel.getInstance().stamina <2) {
						showBuyTip();
						return;
						}
				...
				case _robBt: 
					...
					if (CrystalHoleModel.getInstance().stamina <1) {
						showBuyTip();
						return;
						}
					//掠夺冷却时间未到则给出提示
					if (_robCdTime > 0) {
						AlertConfirmPane.getInstance().type = 0;
						AlertConfirmPane.getInstance().showMessage(ErrorZhCn.ERROR_MESSAGE.Message.(@ID == 703).@Text);//<Message ID="703" Text="矿点处于掠夺保护期" />
						return;
						}
					//已完成则给出提示
					if (_occupyTime <= 0 && _mineModel.owner) {
						AlertConfirmPane.getInstance().type = 0;
						AlertConfirmPane.getInstance().showMessage(ErrorZhCn.ERROR_MESSAGE.Message.(@ID == 704).@Text);//<Message ID="704" Text="矿点已经开采完毕" />
						return;
						}
