package project.view.module.event.QQ
{
	...
	import project.view.module.event.rechargeRank.BonusPanel;
	import project.view.module.event.rechargeRank.RankPanel;
	/**
	 * 消费排名活动面板
	 * @author bbjxl 2013
	 */
	public class QQRechargeRandkEventPanel extends EventBase
	{
		...
		private var _rankPanel:QQRankPanel;
		//-----------------------数据---------------------------------
		...
		private var _ranks:Vector.<QQRechargeRank>;
		private var _callbackObj:*;
		private var _levelArr:Array = [1, 5, 10, 20, 30, 50, 80, 100, 150];
		
		//-----------------------事件--------------------------------
		public function QQRechargeRandkEventPanel()
		{
		...
		private function init():void
		{
			...
			_ui = new QQRechargeRankEventPanelUI();
			addChild(_ui);
		...
		private function updataUI(param:Object,cards:Array):void
		{
			...
			_crystalText.text = _crystal.toString()+"Q点";
			...
			_ranks = new Vector.<QQRechargeRank>();
			for (var i:int = 0; i < param.ranks.length; i++)
			{
				var tempRank:QQRechargeRank = new QQRechargeRank();
			...
			_rankPanel.QQsetData(_ranks, _name,_crystal,_minCrystal,_callbackObj);
