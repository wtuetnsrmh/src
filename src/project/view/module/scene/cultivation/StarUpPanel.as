package project.view.module.scene.cultivation
{
	...
	public class StarUpPanel extends BaseSpriteSetUI
	{
		...
		//-----------------------数据---------------------------------
		private var _luckRatio:Number = 0;//祝福石概率
		...
		private var _currentSelectedClass:int = 0;//当前选择的类别
		private var _currentSelectedPage:int = 1;//当前选择的页码
		//-----------------------事件--------------------------------
		...
		public function updata():void {
			...
			_tabNav.selectedByIndex(_currentSelectedClass);
		...
		private function configerInstance():void
		{
			...
			_ratioText = getChild("ratioText") as TextField;
			...
			_controlPage.setLocation(497, 599);
			...
			switch(_checkBox.currentSelectOpion) {
				...
				case 1:
					...
					UseItemControler.judgestItemExist(SpecialItemIdConfig.LUCKY, function (param:ItemModel):void {
						_itemIco.ico = param.ico;
						luckRatio = LocalConfig.LOCAL_LAN.MixPane.@ratio;
						//-------------------新手引导用-----------------------
			...
			SMZTController.getInstance().cardStarUp(_currentHitCard.card.cardId, function (param:int):void {
				...
				switch(param) {
					case 0:
						...
						PlayerData.getInstance().addOneCard(findPreStarCardId(_currentHitCard.card.cardId));//增加一张卡
					...
					case 2:
						...
						//trace(_currentHitCard.card.cardId);
						//trace(findPreStarCardId(_currentHitCard.card.cardId));
		 ...
		 * @param	e
		 */
		private function findPreStarCardId(value:int):int {
			var _loc_13:XML;
			for each (_loc_13 in ItemConfig.CARD_CONFIG.Card)
            {
                if (_loc_13.StarCard.child(0).toXMLString() == String(value))
                {
					return int(_loc_13.@ID);
				}
			}
			Debug.log("没找到上一星的卡,该卡如果是一星的话是正常")
			return 0;
		}
		...
		private function tabNavigatorChanger(e:SMZTDataChangeEvent):void 
		{
			...
			switch (e.dataObject)
			{
				case _soldrsBt: 
					temp = 0;
					_controlPage.starComputeNoInit(CultivationModel.getInstance().slods, updataUI, 6);
					
					break;
				case _talismanBt: 
					temp = 1;
					_controlPage.starComputeNoInit(CultivationModel.getInstance().tal, updataUI,6);
					break;
				case _mitacBt: 
					temp = 2;
					_controlPage.starComputeNoInit(CultivationModel.getInstance().mit, updataUI,6);
					break;
			}
			if (_currentSelectedClass != temp) {
				//分类有变化
				_currentSelectedClass = temp;
				_currentSelectedPage = 1;
				_controlPage.currentPageNum = _currentSelectedPage;
				return;
				}
			
			if (_currentSelectedPage > _controlPage.totalPageNum) {
				//当前页码比总页码还大时，当前页为总页
				_currentSelectedPage = _controlPage.totalPageNum;
				_currentSelectedPage = (_currentSelectedPage == 0)?1:_currentSelectedPage;
				}
			_controlPage.currentPageNum = _currentSelectedPage;
		}
		...
		private function updataUI(arg:Vector.<CardModel>):void {
			_currentSelectedPage = _controlPage.currentPageNum;
		...
		{
			if (e.target is MovieClipButton) return;//出售按钮点击不可拖
		...
		private function rollOutHandler(e:MouseEvent):void {
			...
			public function get luckRatio():Number 
			{
				return _luckRatio;
			}
			
			public function set luckRatio(value:Number):void 
			{
				_luckRatio = value;
				if (_currentHitCard.card) {
					var tempRatio:String = (_currentHitCard.card.getCardModel().StarUpRate * 100+value) + "%";
					_ratioText.text = String(LocalConfig.LOCAL_LAN.MixPane.StarUpTip.@Text).replace("{ratio}", tempRatio);
					}else {
						_ratioText.text = "";
						}
