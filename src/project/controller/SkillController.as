package project.controller 
{
	...
	{
		public static var _attackPanel:AttackPanel;
		public static var _skillControler:skillTest;
		...
		public static function attackedFunc(timeLen:Number,param:ActionModel, callback:Function = null):void {
			...
			timeline.append( new TweenLite(card, timeLen/2, { x:"3",y:"3"} ));
			timeline.append( new TweenLite(card, timeLen/2, { x:"-6",y:"-6"} ));
			timeline.append( new TweenLite(card, timeLen/2, {x:"3",y:"3", onComplete:function () : void
				{
					if (callback != null) {
						callback();
						}
			...
			switch(param1) {
				case "d":
					...
					if (attackMode == 1) {
						...
						//近战
						 TweenLite.to(card, timeLen, {ease:Quint.easeOut, y:"146", onComplete:function () : void
						{
							...
							mc.y = 416.95;
							_attackPanel.mc_cardArea.addChild(mc);
							TweenLite.to(card, timeLen, {ease:Quint.easeOut, y:oldY,delay:timeLen>>1,onComplete:function () : void
							{
								mc.stop();
