package starling.utils
{
    ...
    public class AssetManager extends EventDispatcher
    {
		...
		private var runtimeLoadTexture:Dictionary;//����ʱ��ʱ���ص����?�������������Ա�����
		...
		/**
		 * <p>��������ʱ����</p>
		 * <p>��ͨ��clearRuntimeLoadTextureһ����������ʱ��̬���ص�����</>
		 * */
		public function loadTexture(path:String,callBack:Function,isCache:Boolean=true):void{
			...
			if(texture){
				...
				if(_countDic[name])_countDic[name]++;
				else trace("出错不应该没有");
				completeLoadIist();
				return;
			}
			...
			function onLoaderComplete(event:Object):void{
				...
				if(isCache) {
					runtimeLoadTexture[name] = texture;
					if(_countDic[name])trace("出错了，不应该有="+_countDic[name]);
					else
						_countDic[name]=1;
				}
				if(callBack) callBack(texture);
		...
		private var _countDic:Dictionary=new Dictionary();//texture计数
		 ...
		 * ����ָ������ʱ��̬���ص�����
		 */		
		public function clearRuntimeLoadTextureByPath(path:String):void {
		var name:String = getName(path);
		
		_countDic[name]--;
		if(_countDic[name]>0){
			trace("还有素质="+_countDic[name]);
			return;
		}
			var texture:Texture = runtimeLoadTexture[name];
				if (texture) {
				texture.dispose();
				texture=null;
				delete runtimeLoadTexture[name];
			}
